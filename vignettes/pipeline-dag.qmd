...
# Wrap a DT data frame or return existing widget
as_dt <- function(x, ...) {
  if (inherits(x, "datatables")) return(x)
  if (is.data.frame(x) && requireNamespace("DT", quietly = TRUE)) {
    return(DT::datatable(x, rownames = FALSE, ...))
  }
  invisible(NULL)
}
```
...
as_dt(res, options = list(pageLength = 15, dom = "t", order = list(list(1, "desc"))))
```
...
if (inherits(res, "visNetwork")) {
  res
} else if (is.list(res) && "nodes" %in% names(res) &&
           requireNamespace("visNetwork", quietly = TRUE)) {
  visNetwork::visNetwork(
    res$nodes, res$edges, main = res$main, height = "600px", width = "100%"
  ) |>
...
as_dt(res, options = list(pageLength = 10, dom = "tip", order = list(list(0, "desc"))))
```
