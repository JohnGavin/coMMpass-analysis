...
```{r manifest, eval=targets_available}
...
```{r plot_types, fig.width=8, fig.height=4, eval=targets_available}
...
```{r samples, eval=targets_available}
downloaded <- safe_tar_read("rna_sample_files")
if (!is.null(downloaded)) print(basename(downloaded))
```
...
```{r clinical, eval=targets_available}
