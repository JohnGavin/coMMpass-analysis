...
**NEVER call btw_tool_run_r directly for:** - devtools::test(), check(),
build() → Use agent with `model="sonnet"` - Any gh::gh() API calls that
might hang → Use `Bash` with timeout - Any operation expecting \>10
lines output → Use agent with `model="sonnet"` - Debugging test failures
→ Use `r-debugger` agent - shiny::runApp() or launch_dashboard() → WILL
HANG (waits for browser) - Any function that might wait for user input →
